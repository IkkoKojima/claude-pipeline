---
name: impl
description: issue を「計画 → 計画レビュー (Codex / Fable 代替) → 実装 (Opus サブエージェント) → 検証 → PR → 自動マージ」まで自動で通す。引数は issue 番号 (複数可)。クラウドセッション (impl 環境) で sweep から呼ばれるのが通常。設定は pipeline.toml、規約は CLAUDE.md の ## pipeline 節
---
...
| 計画レビュー | Codex (`models.plan_review`) を `codex_review.sh` で ≤5 往復。API 制限時は **サブエージェント `pipeline:plan-reviewer`** (Fable、新コンテキスト) |
| 実装 | **サブエージェント `pipeline:implementer`** (Opus)。計画との齟齬・新事実はセッション本体にエスカレーション |
...
- `PIPELINE_ENV=deploy` なら何もせず終了 (deploy 環境では impl を動かさない)
...
- 禁止領域: `$PC forbidden` (既定 + `verify.forbidden_paths`) と CLAUDE.md `## pipeline` の記述。触らないと実現できない要件は blocked
...
`origin/main` から `claude/task-$N-<slug>` を切る (英小文字とハイフン、20 字以内)。issue と関連コードを読み、`.pipeline/plan-$N.md` を書く:
...
- 計画は **issue のコメント**として残す (先頭行 `<!-- pipeline:plan -->`、`$GH comment $N .pipeline/plan-$N.md`)。改訂のたびに新しいコメントを追加 (履歴になる)
- 破壊的変更 (既存バイナリ・既存データが壊れる migration / API) が要るなら blocked

## 3. 計画レビュー (≤5 往復)

```bash
bash $KIT/scripts/codex_review.sh plan <round> .pipeline/plan-$N.md $N        # VERDICT=approved|revise|skipped, REASON=
```
...
## 4. 実装 (Agent ツールで `pipeline:implementer`) とエスカレーション

渡すもの (自己完結で): issue 番号とタイトル、計画の全文、作業ブランチ名、禁止領域の一覧、検証コマンド
`bash $KIT/scripts/verify.sh` (`.pipeline/verify/` にログ)、CLAUDE.md の規約を読むこと、コミットの流儀 (`git add -A` 禁止、節目ごとに commit + push)。
...
```bash
bash $KIT/scripts/verify.sh                       # 変更ファイルから pipeline.toml の検証を選んで実行。最終行 VERIFY=ok|fail
```
...
1. PR 本文 `.pipeline/pr-$N.md` を `$KIT/templates/pr-body.md` の構成で書く (`Refs #N`、変更点 / テスト結果 / 実機確認の観点 / 判断した点 / Codex 指摘の採否 / Codex 往復)
2. `python3 $KIT/scripts/verify_checklist.py check --pr-body .pipeline/pr-$N.md` が exit 0 になるまで直す (禁止領域もここで弾かれる)
3. `git push` → MCP `create_pull_request` (base `main`、head `claude/task-$N-<slug>`、title `<要旨> (#N)`、body = PR 本文)
4. `git fetch origin main && git merge origin/main` (競合は解消) → **最終 head で** `verify.sh` を再実行 → 結果と head SHA を PR 本文に書き `gh api -X PATCH $R/pulls/<pr> -F body=@.pipeline/pr-$N.md`、push
...
| 結果 | 操作 |
|---|---|
| マージ | `label-add merged_unverified`、`label-del ready`、`label-del in_progress`。コメント: PR 番号 + 「実機確認の観点」「判断した点」「Codex 指摘の採否」「Codex 往復」を PR 本文から転記。**issue は close しない** (実機確認の OK を `/pipeline:release` が処理するまで open のまま) |
