...
# 実行計画は pipeline_config.py plan が pipeline.toml (stacks / verify_always / verify_paths /
...
#
# 環境変数:
#   PIPELINE_VERIFY_TIMEOUT  各ステップ全体の上限 (timeout(1) の書式。例 1800 / 30m)。既定なし
#                            (preset のコマンドは自前の `timeout` を持っている)
#   PIPELINE_PYTHON          python 実行ファイル (既定 python3)
#
...
#
# 出力: 各ステップの出力を stdout にそのまま流し、.pipeline/verify/<n>-<stack>-<kind>.log にも保存する
#   (実行前に同ディレクトリの古い *.log は消す)。最後に表 (summary.txt にも保存) と 1 行:
#     VERIFY=ok|fail STEPS=<n> FAILED=<m>
#   失敗が 1 つでもあれば exit 1 (途中で止めず全ステップ回す)。pipeline.toml が無いときも exit 1。
# =============================================================================
set -u
...
if [ ! -f pipeline.toml ]; then
  echo "verify.sh: $ROOT/pipeline.toml が無い (pipeline_config.py init で作る)" >&2
  echo "VERIFY=fail STEPS=0 FAILED=0"
  exit 1
fi

LOG_DIR=".pipeline/verify"
mkdir -p "$LOG_DIR"
...
TIMEOUT_PREFIX=()
if [ -n "${PIPELINE_VERIFY_TIMEOUT:-}" ]; then
  if command -v timeout >/dev/null 2>&1; then
    TIMEOUT_PREFIX=(timeout "$PIPELINE_VERIFY_TIMEOUT")
  else
    echo "verify.sh: 警告: timeout コマンドが無いので PIPELINE_VERIFY_TIMEOUT を無視する" >&2
  fi
fi
...
for ((i = 0; i < N; i++)); do
  ...
  if [ "$rc" -eq 0 ]; then
    ...
    if [ "$rc" -eq 124 ] && [ -n "${TIMEOUT_PREFIX[*]+x}" ]; then
      echo "# TIMEOUT: PIPELINE_VERIFY_TIMEOUT=$PIPELINE_VERIFY_TIMEOUT を超えた" | tee -a "$log"
    fi
  fi
