...
#
# 設定: pipeline.toml の [[release.providers]] のうち type が一致する最初の項目。PROVIDER_INDEX=<n> (0 始まり) で
...
cfg()   { "$PY" "$SCRIPTS_DIR/pipeline_config.py" ${PIPELINE_ROOT:+--root "$PIPELINE_ROOT"} "$@" | tr -d '\r'; }
...
load_entry() {
  local all
  all="$(cfg get release.providers --default '[]')" || die "pipeline_config.py を実行できない"
...
provider_usage() {
  ...
  echo "  設定は pipeline.toml の [[release.providers]] (type = \"$PROVIDER_TYPE\")。PROVIDER_INDEX=<n> で項目を明示" >&2
}
