...
スクリプトではなく **`/pipeline:release` のセッション本体が読んで従う手順**。本番 DB への書き込みは MCP ツール
...
3. **承認バンドル** — `.pipeline/supabase-bundle.md` を 1 ファイルにまとめる:
...
4. **提示** — `sha256sum .pipeline/supabase-bundle.md | cut -c1-8` を添えてバンドル全文を owner に見せ、
