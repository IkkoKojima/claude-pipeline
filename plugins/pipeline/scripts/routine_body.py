...
既定値は pipeline.toml: name = "<repo 名> sweep"、cron = sweep.cron (UTC)、model = models.session。
...
def repo_slug(cfg: dict, override: str | None) -> str:
    ...
    if not _SLUG.match(s):
        die("repo (owner/name) が決まらない。pipeline.toml に repo = \"owner/name\" を書くか --repo を渡す")
    return s
...
def main(argv: list[str] | None = None) -> int:
    ...
    def job_args(p: argparse.ArgumentParser) -> None:
        ...
        p.add_argument("--repo", help="owner/name (既定 pipeline.toml の repo > git remote)")
    ...
    status_title = str(cfg.get("status_issue_title") or "#pipeline-status")
