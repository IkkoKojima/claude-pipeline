#!/bin/bash
# =============================================================================
# claude-pipeline: stack flutter の SessionStart 処理 (scripts/session_start.sh から呼ばれる)
# 入力 (環境変数): STACK_NAME / STACK_PATH (repo ルートからの相対) / STACK_OPTS (opts の JSON) /
#   PIPELINE_ROOT / PIPELINE_KIT / PIPELINE_LOG_DIR / PIPELINE_REPO (owner/name)
# 処理:
#   1) native hook ビルド成果物のキャッシュ (opts.native_cache_tag。空 = 既定なら skip) を GitHub Release から
#      REST で取得 → PIPELINE_ROOT/cache/<tag>/ に展開 → .dart_tool/hooks_runner に復元
...
ROOT="${PIPELINE_ROOT:-/opt/pipeline}"
KIT="${PIPELINE_KIT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
LOG_DIR="${PIPELINE_LOG_DIR:-${TMPDIR:-/tmp}}"
LTAG="flutter-$(printf '%s' "${STACK_PATH:-.}" | tr -c 'A-Za-z0-9' '_')"
say() { echo "[pipeline:flutter] $*"; }
...
if [ -n "$TAG" ]; then
  ...
  if [ ! -d "$CACHE_DIR/hooks_runner" ]; then
    ...
    REPO="${PIPELINE_REPO:-$(python3 "$KIT/scripts/pipeline_config.py" repo 2>/dev/null)}"
