# =============================================================================
# claude-pipeline: stack python の setup (uv + 任意の Python の pin)
#
...
# apt の共有ライブラリ (例: libportaudio2) は pipeline.toml の env.extra_apt に書く (bootstrap が入れる)。
...
pl_setup_python() {
  ...
  if [ -n "$UV" ]; then
    ...
    curl -LsSf https://astral.sh/uv/install.sh 2>/dev/null \
      | env UV_INSTALL_DIR="$PIPELINE_BIN" UV_NO_MODIFY_PATH=1 sh >>"$PL_LOG" 2>&1
    if [ ! -x "$PIPELINE_BIN/uv" ]; then
    ...
    UV="$(command -v uv 2>/dev/null)"; [ -n "$UV" ] || UV="$PIPELINE_BIN/uv"
