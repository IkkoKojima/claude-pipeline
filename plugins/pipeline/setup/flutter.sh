# =============================================================================
# claude-pipeline: stack flutter の setup (Flutter SDK + Android SDK)
#
# opts (pipeline.toml の [[stacks]] に同名キーで書くと上書き。env_api.py render が置換する):
...
pl_setup_flutter() {
  ...
  local FLUTTER_HOME="$PIPELINE_ROOT/flutter" ANDROID_HOME="$PIPELINE_ROOT/android-sdk"
  ...
  (
    ...
    curl -fsSL "$URL" | tar -xJ -C "$PIPELINE_ROOT" && [ -x "$FLUTTER_HOME/bin/flutter" ] \
  ...
  if [ -n "$ANDROID_PLATFORM" ]; then
    (
      ...
      ZIP="$PIPELINE_ROOT/cmdline-tools.zip"
