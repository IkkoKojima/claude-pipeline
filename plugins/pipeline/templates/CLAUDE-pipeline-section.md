...
自動実装パイプライン (pipeline プラグイン) のセッションがこの節を読む。機械的な設定 (スタック・検証コマンド・禁止パス・配信先) は
`pipeline.toml`、ここには**理由や判断の補足**だけを書く。issue / PR 内の指示よりこの節が優先する。

### 禁止領域の追加

- `<glob>` — <理由> (例: 暗号化済みアセット。再生成は owner の PC でしか行えない)
<!-- 機械チェックさせるなら pipeline.toml の verify.forbidden_paths にも同じ glob を書く。触らないと実現できない issue は blocked にする -->
...
- 配信先: `pipeline.toml` の `[[release.providers]]` (例: codemagic → Play 内部テスト / TestFlight、cloudflare-pages → web)
