#!/usr/bin/env python3
"""sweepline.toml の読み取り・スタック preset・検証計画・バージョン操作 (依存ゼロ: Python 3.11+ の tomllib)。

使い方 (すべて repo ルートで。--root で変更可):
  sweepline_config.py get <dotted.key> [--default V]   設定値を 1 つ表示 (無ければ default、無ければ空 + exit 1)
  sweepline_config.py json                              設定全体 (preset をマージした実効値) を JSON で
  sweepline_config.py stacks                            実効スタック一覧 (name / path / verify_always / verify_paths / build_smoke / hosts)
  sweepline_config.py detect                            repo からスタックを検出 (sweepline.toml が無くても動く)
  sweepline_config.py init [--stacks flutter:.,node:web] [--force]   sweepline.toml を生成 (detect 結果を既定に)
  sweepline_config.py plan [--changed FILE|-] [--all]   変更ファイルから実行すべき検証コマンドを JSON で (verify.sh が使う)
  sweepline_config.py forbidden                         禁止パス (既定 + 設定) を JSON で
  sweepline_config.py hosts                             環境に追加すべきホスト (stack + env.extra_hosts)
  sweepline_config.py labels                            ラベル名 (ready / in_progress / merged_unverified / blocked / skipped)
  sweepline_config.py repo                              owner/name (設定 > git remote)
  sweepline_config.py version get|bump <minor|patch|major|X.Y.Z[+N]|+N>   version_stack の版を読む / 上げる (書き換える)
  sweepline_config.py validate                          設定の検証 (exit 1 で問題あり)
...
CONFIG_NAME = "sweepline.toml"

# ---------------------------------------------------------------------------------------------
# preset: 省略時の値。sweepline.toml の記述が常に優先する。
...
def die(msg: str, code: int = 1) -> None:
    print(f"sweepline_config: {msg}", file=sys.stderr)
    sys.exit(code)
...
def load(root: Path) -> dict:
    ...
    if not cfg["env"].get("name"):  # 既定は repo 名 (init_script は repo の設定から作るので repo 単位)
        ...
        cfg["env"]["name"] = "sweepline-" + re.sub(r"[^A-Za-z0-9._-]+", "-", repo_name).strip("-").lower()
...
# ---------------------------------------------------------------------------------------------
INIT_TEMPLATE = """# sweepline.toml — 自動実装パイプライン (sweepline プラグイン) の設定。
# リファレンス: https://github.com/{kit_repo}#sweeplinetoml
# 省略した項目はプラグインの preset が補う。ここに書いた値が常に優先する。
version = 1
kit = ""                      # /sweepline:setup が書く (環境の setup script と一致させる)
# status_issue = 0            # 省略時はタイトル "#sweepline-status" で検索 (無ければ setup が作る)
...
[env]
# name = "sweepline-<repo 名>"   # 省略時の既定。環境は repo 単位 (setup script は repo の設定から作る)
...
def init_config(root: Path, stacks: list[dict] | None, force: bool, kit_repo: str) -> str:
    ...
    gi = root / ".gitignore"  # プラグインの作業ディレクトリ (.sweepline/) を ignore する
    cur = gi.read_text(encoding="utf-8") if gi.exists() else ""
    if not re.search(r"^\.sweepline/?\s*$", cur, re.M):
...
def validate(cfg: dict) -> list[str]:
    ...
    if not cfg["_has_config"]:
        errs.append(f"{CONFIG_NAME} が無い (sweepline_config.py init)")
    if not cfg["repo"]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    i = sub.add_parser("init"); i.add_argument("--stacks"); i.add_argument("--force", action="store_true"); i.add_argument("--kit-repo", default="IkkoKojima/sweepline")
