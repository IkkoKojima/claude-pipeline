...
p_preflight() {
  ...
  acc="$(cf_account)"
  [[ -n "$acc" ]] || preflight_ng "CLOUDFLARE_ACCOUNT_ID が無い (環境変数か sweepline.toml の account_id)"
  token="$(cf_token)" || preflight_ng "CLOUDFLARE_API_TOKEN が無い (secrets_list を参照)"
  dir="$(pget dir)"
  project="$(pget project)"
  [[ -n "$dir" && -n "$project" ]] || preflight_ng "dir と project が必要 (sweepline.toml の [[release.providers]] type=cloudflare-pages)"
...
p_secrets_list() {
  ...
  if [[ -z "$(pget account_id)" ]]; then
    kv SECRET "CLOUDFLARE_ACCOUNT_ID|env_var|Cloudflare の account id (秘密ではない。sweepline.toml の account_id に書けば不要)"
  fi
}
