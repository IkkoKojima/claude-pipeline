...
p_preflight() {
  local wf slug state
  wf="$(pget workflow)"
  [[ -n "$wf" ]] || preflight_ng "workflow が無い (sweepline.toml の [[release.providers]] type=actions-dispatch)"
...
p_secrets_list() {
  ...
  if (( found == 0 )); then
    kv NOTE "workflow が使う秘密は repo の Actions secrets に置く (sweepline.toml の secrets に列挙すると一覧に出る)"
  fi
}
