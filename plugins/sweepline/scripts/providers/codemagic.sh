...
p_preflight() {
  ...
  app="$(cm_app)"
  [[ -n "$app" ]] || preflight_ng "app_id が無い (sweepline.toml の app_id か環境変数 CODEMAGIC_APP_ID)"
  wfs="$(pget workflows)"
  [[ -n "$wfs" ]] || preflight_ng "workflows が無い (sweepline.toml の [[release.providers]] type=codemagic)"
  cm_headers
...
p_deploy() {
  ...
  app="$(cm_app)"
  [[ -n "$app" ]] || die "app_id が無い (sweepline.toml の app_id か環境変数 CODEMAGIC_APP_ID)"
  while IFS= read -r -d '' wf; do
    [[ -n "$wf" ]] && wfs+=("$wf")
  done < <(pget_list workflows)
  (( ${#wfs[@]} > 0 )) || die "workflows が無い (sweepline.toml の [[release.providers]] type=codemagic)"
...
p_secrets_list() {
  ...
  if [[ -z "$(pget app_id)" ]]; then
    kv SECRET "CODEMAGIC_APP_ID|env_var|Codemagic の app id (/sweepline:setup --deploy が POST /apps で作る。sweepline.toml の app_id に書けば不要)"
  fi
