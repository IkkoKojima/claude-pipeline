#!/bin/bash
# release_notes.sh — リリースノート管理 (sweepline プラグイン版。pokemonitor の .claude/skills/deploy/release_notes.sh の移植)。
#
...
#
# version 省略時は sweepline_config.py version get (release.version_stack の版)。notes_dir は sweepline.toml の
...
if [[ -n "${SWEEPLINE_ROOT:-}" ]]; then
  REPO_ROOT="$SWEEPLINE_ROOT"
else
  REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
fi
cd "$REPO_ROOT"
export SWEEPLINE_ROOT="$REPO_ROOT"   # provider の子プロセスも同じ設定を読む
...
version_or_current() {
  ...
  [[ -n "$v" ]] || die "版を決められない (sweepline.toml の release.version_stack を確認するか <version> を渡す)"
  printf '%s' "$v"
}
# X.Y.Z+N の N (無ければ空)
