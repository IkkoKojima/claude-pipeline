...
#
# パイプライン (/sweepline:impl) が使うのは **plan だけ** (v4.2: 実装コードのレビューは行わない)。
# code は手動・将来用に残している。
#
# 計画の入力:
#   <計画 Markdown>   ファイルパス
#   --issue-comment   issue のコメントのうち `<!-- sweepline:plan -->` を含み、author_association が
#                     OWNER / MEMBER / COLLABORATOR のものの **最後の 1 件** (gh api REST で取得)
#
# 設定 (sweepline.toml、sweepline_config.py 経由):
#   repo                 owner/name (取れなければ環境変数 SWEEPLINE_REPO)
...
#   (既定 = plan のモデル)。
#
# 出力: レビュー本文は $SWEEPLINE_REVIEW_DIR (既定 .sweepline/reviews、consumer 側で gitignore) に
...
cd "$REPO_ROOT" || exit 0
OUT_DIR="${SWEEPLINE_REVIEW_DIR:-$REPO_ROOT/.sweepline/reviews}"; mkdir -p "$OUT_DIR"
TIMEOUT_S="${CODEX_TIMEOUT:-1200}"
MAX_DIFF_BYTES="${CODEX_MAX_DIFF_BYTES:-400000}"

cfg() { "$PY" "$KIT/scripts/sweepline_config.py" --root "$REPO_ROOT" "$@" 2>/dev/null | tr -d '\r'; }
ghapi() { MSYS_NO_PATHCONV=1 gh api "$@"; }

SLUG="$(cfg repo)"; [ -n "$SLUG" ] || SLUG="${SWEEPLINE_REPO:-}"
...
tree_fingerprint() { git status --porcelain 2>/dev/null | grep -v '^?? \.sweepline/' | _sha1 | cut -c1-12; }
...
if [ "$MODE" = plan ]; then
  if [ "$TARGET" = "--issue-comment" ]; then
    ...
    [ -n "$SLUG" ] || finish skipped "repo slug unknown (sweepline.toml repo / git remote / SWEEPLINE_REPO)"
...
- Respect the project conventions in CLAUDE.md (including its \"## sweepline\" section), README.md and the docs they reference (read them if relevant to the change).
...
# 待っても通らないので、/sweepline:impl は REASON=no_api_key を api_limit と同じ扱い (Fable 代替) にする
...
if [ $rc -ne 0 ]; then
  # API 制限 (429 / quota / 課金上限 / 認証切れ) は REASON=api_limit で返し、/sweepline:impl が Fable サブエージェントに切り替えられるようにする
