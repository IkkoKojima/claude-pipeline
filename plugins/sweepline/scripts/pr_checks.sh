...
#
# sweepline.toml の [merge] wait_for_checks = ["ci-ok", ...] (check-run 名 or commit status の context) を、
...
cfg() { "$PY" "$KIT/scripts/sweepline_config.py" --root "$ROOT" "$@" 2>/dev/null | tr -d '\r'; }
...
[ -n "$SLUG" ] || err "repo slug unknown (sweepline.toml repo / git remote / SWEEPLINE_REPO)"
