...
1. プラグインのスキル `sweepline:sweep` を Skill ツールで実行し、その手順どおりに最後まで進める。
   Skill ツールで見つからない場合は `${CLAUDE_PLUGIN_ROOT}/skills/sweep/SKILL.md` を読んで従う
   (変数が空なら `find ~/.claude/plugins -path '*sweepline*/skills/sweep/SKILL.md' 2>/dev/null | head -n1` で探す)。
   どちらも無ければ環境の setup が壊れているので、5 の固定 issue に「sweep 起動障害: sweepline プラグインが無い」とコメントして終了する。
