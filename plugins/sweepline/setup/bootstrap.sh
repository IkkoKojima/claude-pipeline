# =============================================================================
# sweepline: 環境 setup script の共通部 (bootstrap)
#
# env_api.py render が次の順に連結して、クラウド環境の init_script (Setup script) にする:
#   ヘッダ (set -u / KIT_REPO / KIT_SHA / SWEEPLINE_ROOT / EXTRA_APT を export)
...
#   - ここで定義する log / pl_link / pl_env / SWEEPLINE_BIN / PL_ENV_FILE / PL_LOG / P_BOOT_* を使ってよい。
#   - 重い DL は `( ... ) &` で並列にし、依存するものだけ wait する。残りは finish.sh が wait する。
#   - apt-get を使うのはここ (P_BOOT_APT) だけ。stack 側の apt 依存は sweepline.toml の env.extra_apt に書く。
# =============================================================================
T0=$(date +%s)
...
SWEEPLINE_ROOT="${SWEEPLINE_ROOT:-/opt/sweepline}"
if ! mkdir -p "$SWEEPLINE_ROOT" 2>/dev/null || [ ! -w "$SWEEPLINE_ROOT" ]; then
  SWEEPLINE_ROOT="${HOME:-/root}/sweepline"; mkdir -p "$SWEEPLINE_ROOT" 2>/dev/null || true
fi
export SWEEPLINE_ROOT
PL_LOG="$SWEEPLINE_ROOT/setup.log"
log() { echo "[sweepline-setup +$(( $(date +%s) - T0 ))s] $*" | tee -a "$PL_LOG"; }

# 非ログインシェル (Bash ツール) からも見えるよう、各ツールは SWEEPLINE_BIN に symlink する
SWEEPLINE_BIN=/usr/local/bin
if [ ! -w "$SWEEPLINE_BIN" ]; then SWEEPLINE_BIN="${HOME:-/root}/.local/bin"; mkdir -p "$SWEEPLINE_BIN" 2>/dev/null || true; fi
# pl_link <実体> [名前] : SWEEPLINE_BIN/<名前> -> <実体> (実体が無ければ何もせず 1 を返す)
pl_link() { [ -e "$1" ] && ln -sfn "$1" "$SWEEPLINE_BIN/${2:-$(basename "$1")}" 2>/dev/null; }
# pl_env <行>... : env.sh に行を足す (SessionStart hook と /etc/profile.d/sweepline.sh が読む)
PL_ENV_FILE="$SWEEPLINE_ROOT/env.sh"
pl_env() { printf '%s\n' "$@" >> "$PL_ENV_FILE"; }

log "start: root=$SWEEPLINE_ROOT kit=${KIT_REPO:-?}@${KIT_SHA:-?} user=$(id -un) extra_apt='${EXTRA_APT:-}'"

# env.sh は毎回作り直す (stack テンプレートが後ろに追記する)
{
  echo "# sweepline: 環境の setup script が生成 ($(date -u +%FT%TZ))。手で編集しない"
  echo "export SWEEPLINE_ROOT=\"$SWEEPLINE_ROOT\""
  echo "export PATH=\"$SWEEPLINE_BIN:\$PATH\""
} > "$PL_ENV_FILE"
...
(
  ...
  if [ -n "${KIT_SHA:-}" ] && [ "$(cat "$KIT_DIR/.sha" 2>/dev/null)" = "$KIT_SHA" ] && [ -f "$KIT_DIR/.claude-plugin/marketplace.json" ]; then
    ...
    TGZ="$SWEEPLINE_ROOT/kit.tgz"; NEW="$KIT_DIR.new"
    rm -rf "$NEW"; mkdir -p "$NEW"
    if curl -fsSL -o "$TGZ" "https://codeload.github.com/${KIT_REPO:-IkkoKojima/sweepline}/tar.gz/$REF" \
  ...
  if [ -f "$KIT_DIR/.claude-plugin/marketplace.json" ] && [ -x "$CLAUDE_BIN" ]; then
    ...
    if grep -qs "\"sweepline@$MKT\"" "${HOME:-/root}/.claude/plugins/installed_plugins.json"; then
      log "kit: plugin sweepline@$MKT already installed"
    else
      timeout 120 "$CLAUDE_BIN" plugin install "sweepline@$MKT" >>"$PL_LOG" 2>&1 \
        && log "kit: plugin sweepline@$MKT installed" || log "kit: plugin install FAILED (sweepline@$MKT via $CLAUDE_BIN)"
    fi
  else
