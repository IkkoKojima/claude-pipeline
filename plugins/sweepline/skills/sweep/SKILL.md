---
name: sweep
description: routine が呼ぶ無人運転の入口。リリースロック → main の健全性 → 中断 issue の回収 → ready な issue を番号順に最大 sweep.max_issues 件 /sweepline:impl → 固定 issue に要約。引数 --dry-run (選択と健全性だけ報告)。routine-fire-payload の "issues: N ..." 行があればその issue だけを対象にする
---
