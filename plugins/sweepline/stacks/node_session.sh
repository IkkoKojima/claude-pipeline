#!/bin/bash
# =============================================================================
# sweepline: stack node の SessionStart 処理 (scripts/session_start.sh から呼ばれる)
# 入力 (環境変数): STACK_NAME / STACK_PATH / STACK_OPTS / SWEEPLINE_LOG_DIR
# package-lock.json があれば STACK_PATH で npm ci。出力は SWEEPLINE_LOG_DIR に書き、stdout は 1 行だけ。
# =============================================================================
cd "${STACK_PATH:-.}" || exit 0
LOG_DIR="${SWEEPLINE_LOG_DIR:-${TMPDIR:-/tmp}}"
LOG="$LOG_DIR/node-$(printf '%s' "${STACK_PATH:-.}" | tr -c 'A-Za-z0-9' '_')-npm-ci.log"
say() { echo "[sweepline:node] $*"; }
