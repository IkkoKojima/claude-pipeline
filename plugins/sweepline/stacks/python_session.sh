...
if [ -z "$REQ" ]; then
  ...
  # 隔離 venv (.sweepline/venv)。python_version があり uv が使えればその版で作る (無ければ VM の python3)
  VENV=.sweepline/venv; PYVER="$(opt python_version "")"
